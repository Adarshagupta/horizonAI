  ...
  window.ChatSupportWidget = {
    ...
    listenForAgentConnection: function() {
      ...
      const checkAgentConnection = async () => {
        try {
          console.log('🔍 Checking agent connection for:', conversationId, 'at', `${WIDGET_CONFIG.apiUrl}/api/conversation-status/${conversationId}`);
          ...
          if (response.ok) {
            ...
            if (status.status === 'connected' && status.assignedAgent && 
              ...
              console.log('✅ Agent connected! Setting up widget state');
